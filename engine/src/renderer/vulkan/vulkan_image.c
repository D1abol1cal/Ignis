...
void vulkan_cubemap_create(
    ...
    image_create_info.arrayLayers = 6;  // 6 faces for cubemap
    image_create_info.format = format;
    ...
    i32 memory_type = context->find_memory_index(memory_requirements.memoryTypeBits, memory_flags);
    if (memory_type == -1) {
        KERROR("Required memory type not found. Cubemap not valid.");
    }
    ...
    if (create_view) {
        out_image->view = 0;
        vulkan_cubemap_view_create(context, format, out_image);
    }
}
...
void vulkan_cubemap_view_create(
    ...
    view_create_info.subresourceRange.levelCount = 1;
    view_create_info.subresourceRange.baseArrayLayer = 0;
    view_create_info.subresourceRange.layerCount = 6;  // All 6 faces
...
void vulkan_cubemap_transition_layout(
    ...
    if (old_layout == VK_IMAGE_LAYOUT_UNDEFINED && new_layout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
        ...
        KFATAL("Unsupported cubemap layout transition!");
        return;
    }
