...
# Generate numeric-only version for PDBs. Only needed for linking, and
# versiongen.exe may not exist yet (e.g. during a clean, where it's
# already been deleted by the time later assemblies are processed), so
# skip this entirely unless we're actually about to link.
